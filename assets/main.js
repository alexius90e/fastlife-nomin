...
function useDynamicAdapt(type = 'max') {
  ...
  document.querySelectorAll(`[${attrName}]`).forEach((element) => {
    ...
    if (!attr) return;
    const [toSelector, breakpointStr, orderStr] = attr.split(',').map((v) => v.trim());
    const to = document.querySelector(toSelector);
    if (!to) return;
    const order = orderStr === 'first' || orderStr === 'last' ? orderStr : Number(orderStr);
    dNodes.push({
      element,
      parent: element.parentElement,
      to,
      breakpoint: breakpointStr ? Number(breakpointStr) : 767,
      order: order ?? 'last',
      index: [...element.parentElement.children].indexOf(element),
      // кэшируем индекс
    });
  });
  const breakpoints2 = [...new Set(dNodes.map((n) => n.breakpoint))];
  const mediaQueries = breakpoints2.map((bp) => {
    return {
      breakpoint: bp,
      query: `(${type}-width: ${bp}px)`,
      nodes: dNodes.filter((n) => n.breakpoint === bp),
    };
  });
  function handleMediaChange() {
    mediaQueries.forEach(({ query, nodes }) => {
      const matches = window.matchMedia(query).matches;
      nodes.forEach((n) => {
        if (matches && !n.element.classList.contains(className)) {
          moveTo(n);
        } else if (!matches && n.element.classList.contains(className)) {
          moveBack(n);
        }
      });
    });
  ...
  function moveTo(dNode) {
    const { element, to, order } = dNode;
    element.classList.add(className);
    if (order === 'first') {
      to.prepend(element);
    } else if (order === 'last' || order >= to.children.length) {
      to.append(element);
    } else {
  ...
  function moveBack(dNode) {
    const { element, parent, index } = dNode;
  ...
  if (IMask.Masked) {
    ...
    const { mask = void 0, ...instanceOpts } =
      opts instanceof IMask.Masked
        ...
        : isObject$2(opts) && opts.mask instanceof IMask.Masked
          ? opts
          : {};
    if (mask) {
...
class InputMask {
  ...
  */
  ...
  set mask(mask) {
    ...
    masked2.unmaskedValue = this.masked.unmaskedValue;
    this.masked = masked2;
  }
  /** Raw value */
  get value() {
...
class ContinuousTailDetails {
  ...
  appendTo(masked2) {
    return masked2
      ...
      .aggregate(masked2._appendPlaceholder());
  }
  get state() {
    return {
      value: this.value,
...
class ChunksTailDetails {
  ...
  appendTo(masked2) {
    if (!(masked2 instanceof IMask.MaskedPattern)) {
      const tail = new ContinuousTailDetails(this.toString());
      return tail.appendTo(masked2);
    }
    const details = new ChangeDetails();
    for (let ci = 0; ci < this.chunks.length; ++ci) {
      const chunk = this.chunks[ci];
      const lastBlockIter = masked2._mapPosToBlock(masked2.displayValue.length);
      ...
      if (
        ...
        if (
          ...
          masked2._stops.indexOf(stop) >= 0
        ) {
          details.aggregate(masked2._appendPlaceholder(stop));
        }
        chunkBlock = chunk instanceof ChunksTailDetails && masked2._blocks[stop];
      }
      if (chunkBlock) {
        ...
        if (remainChars)
          details.aggregate(
            masked2.append(remainChars, {
              tail: true,
            }),
          );
      } else {
        details.aggregate(
          masked2.append(chunk.toString(), {
            tail: true,
          }),
        );
      }
...
class PatternCursor {
  constructor(masked2, pos) {
    this.masked = masked2;
    this._log = [];
    const { offset, index } =
      masked2._mapPosToBlock(pos) ||
      (pos < 0
        ? // first
          {
            index: 0,
...
class MaskedPattern extends Masked {
  ...
  extractInput(fromPos, toPos, flags) {
    ...
    this._forEachBlocksInRange(fromPos, toPos, (b, _, fromPos2, toPos2) => {
      input2 += b.extractInput(fromPos2, toPos2, flags);
    });
    return input2;
  }
  ...
  _appendPlaceholder(toBlockIndex) {
    ...
    this._blocks.slice(startBlockIndex, endBlockIndex).forEach((b) => {
      if (!b.lazy || toBlockIndex != null) {
        var _blocks2;
        details.aggregate(
          b._appendPlaceholder((_blocks2 = b._blocks) == null ? void 0 : _blocks2.length),
        );
      }
    });
    return details;
    ...
    if ('mask' in opts) {
      ...
      this.compiledMasks = Array.isArray(opts.mask)
        ? opts.mask.map((m) => {
            ...
            const masked2 = createMask({
              overwrite: this._overwrite,
              eager: this._eager,
              skipInvalid: this._skipInvalid,
              ...maskOpts,
            });
            if (expose) this.exposeMask = masked2;
            return masked2;
          })
        : [];
    }
...
MaskedDynamic.DEFAULTS = {
  ...Masked.DEFAULTS,
  dispatch: (appended, masked2, flags, tail) => {
    if (!masked2.compiledMasks.length) return;
    const inputValue = masked2.rawInputValue;
    const inputs = masked2.compiledMasks.map((m, index) => {
      const isCurrent = masked2.currentMask === m;
      ...
      m.append(appended, masked2.currentMaskFlags(flags));
      m.appendTail(tail);
      return {
        index,
    ...
    return masked2.compiledMasks[inputs[0].index];
  },
};
...
function createPipe(arg, from, to) {
  ...
  const masked2 = createMask(arg);
  return (value) =>
    masked2.runIsolated((m) => {
      m[from] = value;
      return m[to];
    });
}
...
class RepeatBlock extends MaskedPattern {
  ...
  get repeatTo() {
    var _ref2;
    return (_ref2 = Array.isArray(this.repeat) ? this.repeat[1] : this.repeat) != null
      ? _ref2
      : Infinity;
  }
  constructor(opts) {
  ...
  _update(opts) {
    ...
    this.repeat =
      (_ref3 = (_ref4 = repeat != null ? repeat : block.repeat) != null ? _ref4 : this.repeat) !=
      null
        ? _ref3
        : Infinity;
    super._update({
      mask: 'm'.repeat(
        Math.max(
          (this.repeatTo === Infinity &&
            ((_this$_blocks = this._blocks) == null ? void 0 : _this$_blocks.length)) ||
            0,
          this.repeatFrom,
        ),
      ),
...
function loopFix({
  ...
  const activeColIndexWithShift =
    activeColIndex +
    (bothDirections && typeof setTranslate2 === 'undefined' ? -slidesPerView / 2 + 0.5 : 0);
...
const popupButtons = document.querySelectorAll('[data-popup-toggle]');
popupButtons.forEach((popupButton) => {
  const popupName = popupButton.getAttribute(`data-popup-toggle`);
  ...
  popup?.addEventListener('click', (e) => {
    ...
    if (target && !target.closest('.popup__window')) {
...
const masked = document.querySelectorAll('[data-mask]');
if (masked.length > 0) {
  masked.forEach((item) => {
    IMask(item, {
...
const statsVideo = document.querySelectorAll('.video');
if (statsVideo.length > 0) {
  statsVideo.forEach((statsVideo2) => {
    statsVideo2.addEventListener('click', (e) => {
      if (!statsVideo2.controls) {
        statsVideo2.controls = true;
        statsVideo2.play();
      }
    });
  });
}
const cardsArrow = document.querySelector('.how__arrow');
const container = document.querySelector('.how__list');
if (cardsArrow && container) {
  let cards = container.querySelectorAll('.how__item');
  cards = [...cards];
  cardsArrow.addEventListener('click', () => {
    const nextCard = cards.find((card) => {
      return card.offsetLeft > container.scrollLeft + 1;
    });
    if (nextCard) {
...
const tagsBoxToggles = document.querySelectorAll('[data-tags-box-toggle]');
const tagsBox = document.getElementById('tags-box');
if (tagsBoxToggles.length > 0 && tagsBox) {
  tagsBoxToggles.forEach((toggle) => {
    toggle.addEventListener('click', () => {
      tagsBox.classList.toggle('choose-form__tags-box_opened');
    });
  });
}
const prevArrow = document.querySelector('.choose__arrow');
const nextArrow = document.querySelector('.choose__arrow_next');
const row = document.querySelector('.choose__schedule');
if (prevArrow && nextArrow && row) {
...
const scheduleContainer = document.querySelector('.choose__schedule');
const daysCount = 182;
function renderSchedule() {
  ...
  for (let i = 0; i < daysCount; i++) {
    ...
    if (monthNum !== lastMonth) {
      html += `<div class="choose__label-wrapper"><h4 class="choose__label">${monthName}</h4></div>`;
      lastMonth = monthNum;
    }
    html += `<div data-date="${dayNum[0] === '0' ? dayNum[1] : dayNum}-${date.getMonth() + 1}-${date.getFullYear()}" class="choose__day ${isWeekend ? 'choose__day_weekend' : ''}" data-index="${i}">
	<span class="choose__weekday ${isWeekend ? 'choose__weekend' : ''}">${weekDay}</span>
	<span class="choose__date">${dayNum}</span>
</div>`;
  }
...
if (scheduleContainer) {
  let updateSelection = function () {
    const allDays = document.querySelectorAll('.choose__day');
    allDays.forEach((day) => {
      const index = parseInt(day.dataset.index);
      day.classList.remove('_selected', '_in-range');
      day.removeAttribute('data-type');
      if (selectedDates.length === 1 && index === selectedDates[0]) {
        day.classList.add('_selected');
        day.setAttribute('data-type', 'single');
      }
      if (selectedDates.length === 2) {
        if (index === selectedDates[0]) {
          day.classList.add('_selected');
          day.setAttribute('data-type', 'start');
        } else if (index === selectedDates[1]) {
          day.classList.add('_selected');
          day.setAttribute('data-type', 'end');
        } else if (index > selectedDates[0] && index < selectedDates[1]) {
          day.classList.add('_in-range');
        }
      }
    });
  };
  var updateSelection2 = updateSelection;
  renderSchedule();
  let selectedDates = [];
  scheduleContainer.addEventListener('click', (e) => {
    const dayEl = e.target.closest('.choose__day');
    if (!dayEl) return;
    const index = parseInt(dayEl.dataset.index);
    if (selectedDates.length === 0) {
      selectedDates = [index];
    } else if (selectedDates.length === 1) {
      if (index === selectedDates[0]) return;
      selectedDates.push(index);
      selectedDates.sort((a, b) => a - b);
    } else {
      const [start, end] = selectedDates;
      if (index > end) {
        selectedDates[1] = index;
      } else if (index < start) {
        selectedDates[0] = index;
      } else {
        const distToStart = Math.abs(index - start);
        const distToEnd = Math.abs(index - end);
  ...
  const resetBtn = document.querySelector('.choose__cancel');
  if (resetBtn) {
...
const blocks = document.querySelectorAll('.dates__block');
blocks.forEach((block) => {
  const slider2 = block.querySelector('.dates__slider');
  const prevArrow2 = block.querySelector('.dates__arrow_prev');
  const nextArrow2 = block.querySelector('.dates__arrow_next');
  if (slider2) {
    new Swiper(slider2, {
      modules: [Navigation],
      spaceBetween: 10,
...
const unsorted = document.querySelector('[data-dates-unsorted]');
const sorted = document.querySelector('[data-dates-sorted]');
let gender = '';
let age = 0;
let categories = [];
let dates = [];
let touched = false;
const maleChoose = document.getElementById('choose-form-gender-male');
const femaleChoose = document.getElementById('choose-form-gender-female');
const cardsToSort = document.querySelectorAll(
  '.booking-form.booking-form_compact:not(.swiper-slide)',
);
const input = document.getElementById('choose-form-age');
input.addEventListener('input', () => {
  age = +input.value;
  update();
  touched = true;
});
maleChoose.addEventListener('change', () => {
  gender = 'm';
  update();
  touched = true;
});
femaleChoose.addEventListener('change', () => {
  gender = 'f';
  update();
  touched = true;
});
const datesEls = document.querySelectorAll('.choose__day[data-date]');
scheduleContainer.addEventListener('click', (e) => {
  dates = [...datesEls]
    .filter((date) => date.classList.contains('_selected') || date.classList.contains('_in-range'))
    .map((item) => item.dataset.date);
  update();
  touched = true;
});
const categoryTags = document.querySelectorAll('.choose-form__tag input');
categoryTags.forEach((tag) => {
  tag.addEventListener('change', () => {
    categories = [...categoryTags].filter((tag2) => tag2.checked).map((tag2) => tag2.id);
    update();
    touched = true;
  });
});
const reset = document.querySelector('button.choose__cancel');
...
reset.addEventListener('click', () => {
  if (touched) {
    ...
    categories = [];
    dates = [];
    maleChoose.checked = false;
    femaleChoose.checked = false;
    input.value = '';
    datesEls.forEach((item) => {
      item.classList.remove('_selected');
      item.classList.remove('_in-range');
    });
    update();
    touched = false;
    reset.style.display = 'none';
...
function update() {
  ...
  cardsToSort.forEach((card) => {
    const { date, womanAge, manAge, gender: available, category } = card.dataset;
    const [maleMin, maleMax] = manAge.split('-');
    const [femaleMin, femaleMax] = womanAge.split('-');
    const rightGender = available.includes(gender);
    const rightAge =
      age === 0 ||
      (gender === 'm'
        ? age >= +maleMin && age <= +maleMax
        : age >= +femaleMin && age <= +femaleMax);
    const rightDate = dates.includes(date) || dates.length == 0;
    const rightCategory =
      categories.length == 0 || categories.includes(category) || category === '';
    const sort = rightGender && rightAge && rightDate && rightCategory;
    card.style.display = sort ? 'flex' : 'none';
  });
}
update();
...
const pairsSlider = document.querySelector('.pairs__slider');
if (pairsSlider) {
  new Swiper(pairsSlider, {
    modules: [Navigation],
    navigation: {
      prevEl: '.pairs__arrow_prev',
      nextEl: '.pairs__arrow_next',
    },
    slidesPerView: 1,
    spaceBetween: 10,
    //30
...
console.info('Main script executed with code 0');

// media-reviews

const mediaReviewsButtons = document.querySelectorAll('.media-reviews__button');
const mediaReviewsModals = document.querySelectorAll('.media-reviews__modal');

function openReviewsModal(modalEl) {
  modalEl.classList.add('active');
}

function closeReviewsModal(modalEl) {
  modalEl.classList.remove('active');
  const videos = document.querySelectorAll('.media-reviews__modal-main-video iframe');
...
mediaReviewsModals.forEach((modalEl) => {
  modalEl.addEventListener('click', (event) => {
    const isLayout = event.target === event.currentTarget;
    const isClose = event.target.classList.contains('media-reviews__modal-close-button');

    if (isLayout || isClose) closeReviewsModal(event.currentTarget);
  });
});

mediaReviewsButtons.forEach((button) =>
  button.addEventListener('click', (event) => {
    const targetModalId = event.currentTarget.dataset.target;
    const targetModal = document.querySelector(`
      .media-reviews__modal[data-media="${targetModalId}"]
    `);
    if (targetModal) openReviewsModal(targetModal);
  }),
);
